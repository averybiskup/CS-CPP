...
template<class ItemType>
// int LinkedList<ItemType>::replaceItem(ItemType oldValue, ItemType newValue) {
int LinkedList<ItemType>::replaceItem(const ItemType& oldValue, const ItemType& newValue) {
  int i = 0;
  // Iterating over current list
  for (Node<ItemType> *n = headPtr; n; n=n->getNext()) {

    // Changing value if it matches the inputted one
    if (oldValue == n->getItem()) {
      n->setItem(newValue);
      i++;
    }
  }
